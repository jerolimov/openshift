...
export BRIDGE_K8S_MODE_OFF_CLUSTER_ENDPOINT=$(kubectl --context "$KUBECTL_CONTEXT" config view --minify -o jsonpath='{.clusters[0].cluster.server}')
export BRIDGE_K8S_MODE_OFF_CLUSTER_SKIP_VERIFY_TLS=true

export BRIDGE_K8S_AUTH="bearer-token"
export BRIDGE_K8S_AUTH_BEARER_TOKEN=$(kubectl --context "$KUBECTL_CONTEXT" create token -n "$CONSOLE_SERVICE_ACCOUNT_NAMESPACE" "$CONSOLE_SERVICE_ACCOUNT_NAME")
